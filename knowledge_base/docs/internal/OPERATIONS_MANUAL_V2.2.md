---
original_file: "OPERATIONS_MANUAL_V2.2.1"
processed_date: 2026-02-02T23:12:21.668998
type: document_extraction
---

# Document Extraction: OPERATIONS_MANUAL_V2.2.1
...
Version: 2.2.1 (Analyst V1.1 + Outreach Architect V1.0 - Pipeline Validated)  
...
**Output Location:** `knowledge_base/prospects/` (workspace-relative)

**Critical Configuration Note:** As of V1.1, the analyst uses workspace-relative paths (`Path(__file__).parent.parent.parent / "knowledge_base" / "prospects"`). Earlier versions incorrectly used `Path.home()` which wrote outputs to the user's home directory instead of the workspace. This was corrected on February 3, 2026.
...
**Manual Intelligence Injection (Human-in-the-Loop):**

**Issue:** IRS 990 data trails real-time events by 12-24 months. Institutions experiencing acute crises (presidential terminations, no-confidence votes, emergency layoffs) may still show "stable" financials in their most recent tax filings.

**Validated Workflow (February 3, 2026 - Rockland CC Live Test):**
1. Run analyst to generate baseline profile from tax data
2. Conduct CSO intelligence gathering (news research, leadership changes, governance signals)
3. Manually update the generated `[ein]_profile.json`:
   - Set `signals.distress_level` to appropriate level (critical/elevated/watch)
   - Add entries to `signals.indicators[]` array with real-time intelligence
   - Update `financials.calculated.runway_years` if cash flow data available
4. Update corresponding `[name]_dossier.md` to align with profile changes
5. Proceed to Outreach Architect with updated profile

**Example Manual Signal Addition:**
```json
{
  "type": "NEWS",
...
This workflow ensures the Outreach Architect receives accurate distress context for email generation while maintaining the integrity of historical financial analysis.
...
**Pipeline Validation (End-to-End Live Fire Test):**

**Date:** February 3, 2026  
**Test Case:** Rockland Community College (EIN: 13-1969305)  
**Status:** ✅ SUCCESSFUL

**Test Sequence:**
1. CSO intelligence gathering → 4 critical distress signals identified
2. Analyst execution → Profile JSON + Dossier generated (FY2022 tax data)
3. Manual intelligence injection → Updated profile with real-time signals
4. Outreach Architect execution → 3-email critical intervention sequence
5. Quality validation → Zero forbidden phrase violations

**Key Findings:**
- End-to-end execution time: ~50 seconds (Analyst + Outreach)
- Tone enforcement: FLAWLESS (peer-level crisis advisory, zero vendor-speak)
- Manual intelligence integration: REQUIRED for real-time distress context
- Distress escalation logic: Correctly triggered CRITICAL intervention workflow

**Validated Output Quality:**
- Rockland dossier: Executive summary with runway calculation (1.2 years to insolvency)
- Email sequence: 3 emails referencing specific institutional crisis (presidential termination, $8M deficit, 43-0 no-confidence vote)
- Positioning: Successfully deployed "diagnostic wedge" consulting logic (identified contradiction between stable tax data and reported crisis)

**Production Readiness:** The Analyst → Outreach pipeline is battle-tested and ready for client-facing deployment.
...
| **2.2.1** | **2026-02-03** | Corrected Analyst path configuration (workspace-relative), documented manual intelligence injection workflow, added end-to-end pipeline validation results from Rockland CC live fire test |
