---
original_file: "OPERATIONS_MANUAL_V3"
processed_date: 2026-01-31T19:23:59.341810
type: document_extraction
---

# Document Extraction: OPERATIONS_MANUAL_V3

# OPERATIONS_MANUAL.md

## Charter & Stone Automation Stack

**Version:** 3.0 (Architecture V3 — Hybrid Power Automate + MCP)  
...
│       ├── charter_template.docx  # Branded template
│       ├── .env             # Environment configuration
│       └── src/
│           └── _INBOX/      # Drop zone for markdown files
├── shared/
...
│   ├── intelligence/        # Enriched prospect data
│   └── prospects/           # Target institution profiles
├── systemd/
...
├── Governance/              # Architecture, Planner config, Agent registry, Deprecation log
├── Operations/              # Operations manual, procedures
├── Technical/               # MCP reference, technical specs
├── Intelligence/            # Research outputs, dossiers
├── Strategy and Intel/      # Legacy signals (pre-V3)
└── Incoming Signals/        # Power Automate trigger folder (future)
```

**Deprecated:** `sites/Operations` (HQ site) — content migrated to unified site
...
**Critical Filter:**
```python
AGE_THRESHOLD = 30  # days — prevents duplicate alerts on stale news
```
...
**Status:** Production (V3.0 — reconfigured January 31, 2026)
...
# Breach and Clear Protocol — clean stale mounts
...
| Issue | Solution |
|-------|----------|
| Tools not appearing | Full quit Claude Desktop (system tray → Quit), relaunch |
...
| `⚠️ Skipping Teams Alert` | Missing webhook URL | Check `.env` |
| `❌ Error: [Errno 2] No such file` | Template missing | Verify `charter_template.docx` |
...
knowledge_base/
├── signals/
│   └── processed/           # Watchdog output (news signals)
├── docs/
│   └── internal/            # Governance documentation
│       ├── ARCHITECTURE_V3.md
│       ├── PLANNER_CONFIGURATION.md
│       ├── AGENT_REGISTRY.md
│       ├── DEPRECATION_LOG.md
│       ├── MCP_SERVER_REFERENCE.md
│       └── OPERATIONS_MANUAL.md
├── intelligence/            # Enriched 990 data
└── prospects/               # Institution profiles
```
...
- `intelligence` — Financial analysis
- `prospects` — Target institutions
- `all` — Search everything

**Example queries:**
- "Search Oracle for West Virginia" → Finds distress signals
- "Search Oracle for ARCHITECTURE_V3" → Finds governance docs
- "Search Oracle for 990 revenue" → Finds financial intelligence
...
**Signal Tasks:** `🔴 [University] — [Issue Type]`
- 🔴 DISTRESS — Immediate financial/operational crisis
- 🟢 FORECAST — Expansion or investment opportunity
- 🟡 WATCH — Developing situation
- ⚪ INFO — General intelligence
...
- `ARCHITECTURE_V3.md` — System design rationale
- `AGENT_REGISTRY.md` — Digital Teammate specifications
- `DEPRECATION_LOG.md` — What we tried and abandoned
- `MCP_SERVER_REFERENCE.md` — Interactive tooling guide
- `PLANNER_CONFIGURATION.md` — Task management standards
